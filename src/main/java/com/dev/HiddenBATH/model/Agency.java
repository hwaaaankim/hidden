...
@Table(name = "tb_agency",
       indexes = {
           @Index(name = "IX_TB_AGENCY__NAME", columnList = "AGENCY_NAME"),
...
public class Agency {
    ...
    private String sido;       // 원문 시/도 (예: 서울특별시, 경기도)

    @Size(max = 50)
    @Column(name = "SIGUNGU", length = 50)
    private String sigungu;    // 원문 시/군/구 (예: 용인시 수지구, 관악구)

    @Size(max = 50)
    @Column(name = "BNAME", length = 50)
    private String bname;      // 원문 법정동/리 (예: 남현동, 죽전동)
    ...
    // ====== 연락/링크/담당 ======
    @Size(max = 30) @Column(name = "TEL", length = 30)
    private String tel;

    @Size(max = 30) @Column(name = "MOBILE", length = 30)
    private String mobile;

    @Size(max = 300) @Column(name = "KAKAO_TALK_LINK", length = 300)
    private String kakaoTalkLink;

    @Size(max = 100) @Column(name = "STAFF_NAME", length = 100)
